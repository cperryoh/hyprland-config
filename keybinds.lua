...
hl.bind(
	...
	hl.dsp.exec_cmd(
		'wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+ && paplay /usr/share/sounds/freedesktop/stereo/message.oga & quickshell msg "volume-update"'
	),
	{ repeating = true }
)
hl.bind(
...
hl.bind(
	mainMod .. " + " .. shift .. " + P",
	hl.dsp.exec_cmd(
		'sh -c \'f=~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%M-%S).png; hyprshot -m region -o ~/Pictures/Screenshots -f "$(basename "$f")" && wl-copy --type image/png < "$f"\''
	)
)
hl.bind(
	mainMod .. " + " .. shift .. " + W",
	hl.dsp.exec_cmd(
		'sh -c \'f=~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%M-%S).png; hyprshot -m window -o ~/Pictures/Screenshots -f "$(basename "$f")" && wl-copy --type image/png < "$f"\''
	)
)
hl.bind(
	mainMod .. " + " .. shift .. " + 1",
